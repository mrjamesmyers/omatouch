...
// The touchscreen, from svgrepo.com/svg/92802/touch-screen.
//
// A Shape rather than a nerd-font glyph, for the same reason omacar draws its
// car: patched font families disagree about which codepoint holds what, so a
// glyph that looks right today is somebody else's icon after the next theme
// change. The geometry ships with the plugin and takes the theme colour.
//
// TWO THINGS THAT MATTER WHEN FITTING IT.
//
// The artwork is NOT square even though the viewBox is: it occupies x
// 69.768..509.395 of a 579-wide box, so fitting by the viewBox would leave a
// quarter of the width as dead margin and draw the icon smaller than the space
// allows. The item's implicit aspect is the CONTENT's 0.76:1.
//
// And `scale` belongs on the ShapePath, not on the Item. Scaling the Item
// tessellates the curves at source size and then magnifies the result, which
// turns the ripple arcs into visible polygons at panel size.
Item {
    id: root

    property color tint: "#ffffff"

    // The source viewBox, and the box the artwork actually occupies in it.
    readonly property real inkX: 69.768
    readonly property real inkW: 439.627
    readonly property real inkY: 0
    readonly property real inkH: 579.164

    // Exposed so a call site that sets one dimension can derive the other from
    // the artwork instead of guessing.
    readonly property real aspect: inkW / inkH

    implicitHeight: 24
    implicitWidth: Math.round(implicitHeight * aspect)

    // Fit to whichever dimension runs out first, then centre. Scaling by width
    // alone is only right when the box already has the artwork's shape.
    readonly property real k: Math.min(width / inkW, height / inkH)
    readonly property real offX: (width - inkW * k) / 2 - inkX * k
